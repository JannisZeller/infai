...
class HistoryRepo:
    ...
    # async def _add_history(self, history: History):
    #     async with get_session(self._engine) as session:
    #         history_db = HistoryDb(
    #             id=history.id,
    #             created_at=history.created_at,
    #             items=[map_history_item_to_db(item) for item in history.items],
    #         )
    #         session.add(history_db)

    # async def _update_history(self, history: History):
    #     async with get_session(self._engine) as session:
    #         history_db = await self._find_history_db_by_id_eager(history.id)
    #         if not history_db:
    #             raise ValueError(f"History with id {history.id} not found")
    #         history_db.created_at = history.created_at
    #         history_db.items = [map_history_item_to_db(item) for item in history.items]
    #         session.add(history_db)

    # async def add_or_update_history(self, history: History):
    #     history_db = await self._find_history_db_by_id_eager(history.id)
    #     if history_db:
    #         await self._update_history(history)
    #     else:
